...
from api.runner import run_scrapers
from api.services.process_anthropic import process_anthropic_markdown
from api.services.process_youtube import process_youtube_transcripts
from api.services.process_digest import process_digests
from api.services.process_email import send_digest_email
...
def run_daily_pipeline(hours: int = 24, top_n: int = 10, email: list = None) -> dict:
    ...
    try:
        ...
        scrapper_runner(hours, results)

        anthropic_runner(results)

        youtube_runner(results)

        digest_runner(results)

        email_runner(email, hours, results, top_n)
...
def email_runner(email, hours, results, top_n):
    logger.info("\n[5/5] Generating and sending email digest...")
    email_result = send_digest_email(hours=hours, top_n=top_n, email=email)
...
def scrapper_runner(hours, results):
